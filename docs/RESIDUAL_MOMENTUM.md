...
|---|---|---|---|---|---|---|
| Top-6 NDX momentum (the shipped 6-1 book) | 48.1% | 49.2% | 0.96 | −40.4% | 1.19 | 21.1× |
| **Top-6 residual momentum** | **73.6%** | **39.4%** | **1.49** | **−35.3%** | **2.09** | 24.0× |

More return at lower volatility and a shallower drawdown. But the single-cell number is exactly what
...
> when this was run. The default has since moved to 6-1; the single-cell
> comparison above was re-measured against 6-1 and the gap widened rather than
> closed, but **the paired sweep below has not been re-run against 6-1**. Treat
> its counts as evidence about the residual score, not as current to the digit.
...
*(Cost, lag and sub-period tables above are all against the pre-merge 12-1
baseline, for the reason given in the box.)*
